---
type: study
title: "Rillig 2021 \u2014 Early Rhythm Control in HF Patients: EAST-AFNET4 Substudy"
citekey: Rillig2021EastAFNET4
year: 2021
authors: "Rillig A, Magnussen C, Ozga AK, Suling A, Brandes A, Breithardt G, Declerk\
  \ L, Dr\xF6gem\xF6ller T, Eckardt L, Gessler N, Goette A, Gulizia MM, Kuck KH, Leclercq\
  \ C, Loeffler M, Lubinski A, Morsbach F, M\xFCnzel T, Neuberger HU, Schotten U,\
  \ Simantirakis E, Wijeysundera HC, Zapf A, Zeller T, Kirchhof P; EAST-AFNET4 Trial\
  \ Investigators"
journal: Circulation
study_type: pre-specified subgroup analysis (EAST-AFNET4)
evidence_level: high (subgroup)
tags:
- atrial-fibrillation
- rhythm-control
- hfpef
- hfref
- hfmref
- trial
- east-afnet4
created: 2026-05-18
last_updated: 2026-05-18
sources:
- citekey: Rillig2021EastAFNET4
  doi: 10.1161/CIRCULATIONAHA.121.053081
page-type: source-summary-page
---
# Rillig 2021 — Early Rhythm Control in AF+HF: EAST-AFNET4 HF Substudy

> In 798 HF patients from EAST-AFNET4 (56.3% HFpEF), early rhythm control (ERC) significantly reduced the primary cardiovascular outcome vs. usual care (HR 0.74; 95% CI 0.56–0.97; P=0.03), with no significant interaction by HF type (HFpEF vs. HFmrEF vs. HFrEF; P=0.63).

**Full citation:**
Rillig A, Magnussen C, Ozga AK, et al.; EAST-AFNET 4 Investigators. Early rhythm control therapy in patients with heart failure and atrial fibrillation: the EAST-AFNET 4 Trial. *Circulation.* 2021;144(11):845–858. doi:[10.1161/CIRCULATIONAHA.121.053081](https://doi.org/10.1161/CIRCULATIONAHA.121.053081)
**Authors:** Rillig A et al.; EAST-AFNET4 Trial Investigators  
**Year:** 2021 · **Journal:** Circulation 2021;144:845–858  
**Study type:** Pre-specified subgroup analysis of EAST-AFNET4 (NCT01288352)  
**N:** 798 HF patients from the 2,789 total EAST-AFNET4 population  
**NCT:** NCT01288352 (EAST-AFNET4)  
**HF breakdown:** 442 HFpEF (56.3%), 211 HFmrEF (26.4%), 132 HFrEF (16.5%) [remaining 13 unknown]  
**Population:** Early AF (diagnosis ≤1y); age ≥65 or ≥75 with cardiovascular risk factors; HF defined by history/echo/BNP criteria  

---

## Key Findings

### Primary Outcome (HF Subgroup)
- **Primary composite (CV death, stroke, worsening HF, acute coronary syndrome; time to first):** HR 0.74 (95% CI 0.56–0.97; P=0.03) favouring early rhythm control
- **No interaction by HF phenotype:** ERC benefit consistent across HFpEF, HFmrEF, and HFrEF subgroups (P for interaction = 0.63)

### Secondary Outcomes (Exploratory)
- Worsening HF: directionally reduced with ERC in HFpEF
- Stroke: numerically lower with ERC
- CV death: numerically lower with ERC; individual endpoints underpowered
- Hospitalisation for HF: reduced with ERC but not significant in HF subgroup alone

### Context: EAST-AFNET4 Main Results
- Main trial (all patients, N=2,789): ERC reduced primary composite by 21% (HR 0.79; 95% CI 0.66–0.94; P=0.005)
- ERC included both antiarrhythmic drugs AND catheter ablation at physician discretion; ~20% ablation rate in ERC arm

## Methods

Pre-specified analysis of HF subgroup in EAST-AFNET4 (randomised 1:1 to ERC vs. usual care in early AF). HF defined as any of: prior HF hospitalisation, clinical HF diagnosis, reduced LVEF, elevated NPs, or echo diastolic dysfunction. EF subtype classified at enrolment. Primary endpoint identical to main trial. No adjustment for multiple testing; underpowered for individual subgroup endpoints.

## Relevance to HFpEF Wiki

This is the most important trial evidence for rhythm control in AF+HFpEF patients. The HFpEF-predominant subgroup had significant benefit from ERC — comparable to HFmrEF and HFrEF — supporting a class-independent effect. Together with [[alsadawi2022rhythmcontrol]] (meta-analysis showing rhythm control over rate control), this establishes early rhythm control as a promising treatment strategy in AF+HFpEF. The benefit likely derives from AF→sinus rhythm conversion restoring atrial contribution to diastolic filling, which is disproportionately important in HFpEF.

## Connections

- Updates: [[atrial-fibrillation]] — ERC significantly benefits AF+HF including HFpEF; no HF type interaction
- Supports: [[alsadawi2022rhythmcontrol]] — consistent direction and significance for rhythm control benefit
- Connects to: [[wester2023sdb]] — SDB promotes atrial arrhythmia in HFpEF; treating SDB could complement rhythm control strategies
- Note: EAST-AFNET4 is distinct from catheter ablation-only studies; ERC included pharmacological rhythm control

## Related Pages

- Concepts: [[atrial-fibrillation]], [[rhythm-control]]
- Entities: [[atrial-fibrillation]]
- Sources: [[alsadawi2022rhythmcontrol]]

## Contradictions

None. Consistent with meta-analytic data from [[alsadawi2022rhythmcontrol]].
