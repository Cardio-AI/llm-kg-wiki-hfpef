---
type: study
title: Interplay of Chronic Kidney Disease and the Effects of Tirzepatide in HFpEF and Obesity — SUMMIT CKD Subanalysis
citekey: Packer2025SUMMITCKD
year: 2025
authors: Packer M, Zile MR, Kramer CM, et al.; SUMMIT Trial Study Group
journal: J Am Coll Cardiol
study_type: RCT secondary analysis
evidence_level: moderate
tags:
  - trial
  - hfpef
  - glp1ra
  - treatment
  - obesity
  - ckd
created: 2026-05-13
last_updated: 2026-05-13
sources:
  - file: raw/2025-JACC-Packer-SUMMIT_study.pdf
    citekey: Packer2025SUMMITCKD
---
# SUMMIT CKD Subanalysis — Tirzepatide in HFpEF with Obesity by CKD Status

> Tirzepatide benefit in HFpEF with obesity was consistent regardless of CKD status (interaction P=0.86), with substantial weight loss and KCCQ improvement preserved in the 61% of patients who had CKD.

**File:** `raw/2025-JACC-Packer-SUMMIT_study.pdf` · **Authors:** Packer M, Zile MR, Kramer CM, et al. · **Year:** 2025 · **Journal:** J Am Coll Cardiol  
**Study type:** Pre-specified secondary analysis of SUMMIT RCT (NCT04847557) · **N:** 731 total; ~61% with CKD (eGFR <60 mL/min/1.73 m² by cystatin C) · **Population:** HFpEF (LVEF ≥50%), obesity (BMI ≥30), NYHA II–IV  
**Intervention/Exposure:** Tirzepatide 2.5 mg → 15 mg SC weekly vs. placebo (see [[summit]] for full trial details)  
**Primary outcome:** Consistency of tirzepatide treatment effect across CKD/no-CKD strata

---

## Key Findings

The effect of tirzepatide on KCCQ-CSS, body weight, and inflammation was consistent across CKD and no-CKD subgroups (interaction P=0.86). Both subgroups showed substantial weight loss (~13–15% tirzepatide vs. ~2% placebo), KCCQ-CSS improvement (~6–7 pts), and anti-inflammatory effects (hsCRP −35 to −44%). CKD patients represent 61% of this HFpEF population and derive equal benefit from tirzepatide.

This paper also reports the overall SUMMIT primary results for reference: HR 0.62 (0.41–0.95; P=0.026) for CV death/worsening HF and KCCQ-CSS +6.9 pts (3.3–10.6; P<0.001). (source: raw/2025-JACC-Packer-SUMMIT_study.pdf)

## Methods (brief)

Pre-specified subgroup analysis of SUMMIT. CKD defined as eGFR <60 mL/min/1.73m² by cystatin C (not creatinine-based). Baseline: age ~69y, BMI ~38 kg/m², KCCQ-CSS ~52–57, 6MWD ~278–340m, 33–35% AF; SGLT2i use 18–23%. T2DM permitted (HbA1c ≤9.5%).

## Results

**Overall SUMMIT primary results (referenced within this paper):**

| Endpoint | Effect | 95% CI | P |
|---|---|---|---|
| CV death/worsening HF (time-to-first, HR) | 0.62 | 0.41–0.95 | 0.026 |
| KCCQ-CSS change at 52 wk | +6.9 pts | 3.3–10.6 | <0.001 |

**By CKD subgroup:**

| Endpoint | CKD Tirzep | CKD Placebo | No-CKD Tirzep | No-CKD Placebo | Interaction P |
|---|---|---|---|---|---|
| KCCQ-CSS change | +7.0 pts | — | +6.2 pts | — | 0.86 |
| Body weight change | −13.3% | −2.2% | −14.5% | −2.4% | NS |
| hsCRP change | −35.5% | −3.9% | −43.6% | −10.6% | NS |
| 6MWD change | +19m | +10m | +37m | +11m | NS |

GI adverse events: 54.2% tirzepatide vs. 27.5% placebo in CKD subgroup.

## Limitations

- Subgroup analysis; interaction test only excludes qualitative modification; within-subgroup comparisons not independently powered
- CKD defined by cystatin C eGFR, which may select more severely affected renal function patients than creatinine-based criteria
- GI adverse events high in both subgroups; tolerability in CKD requires monitoring in clinical practice
- Primary NEJM paper (Packer 2025) not separately ingested — overall trial results drawn from within this secondary analysis

## Connections

- Supports: [[summit]] — confirms no CKD treatment attenuation; large CKD burden in HFpEF (61%)
- Informs: [[hfpef-phenotype-profiling]] — CKD as co-prevalent comorbidity; benefit unattenuated
- Reassures: [[sglt2-inhibitors]] — tirzepatide used alongside SGLT2i (18–23% background use) without identified safety signal

## Secondary Analyses & Data Reuse

| Citekey | Focus | Wiki page |
|---------|-------|-----------|
| `Packer2025SUMMIT` | Primary RCT (NEJM 2025) | [[packer2025summit]] |
| `Kramer2025SUMMITCMR` | CMR substudy | [[kramer2025summit-cmr]] |

## Related Pages

- Concepts: [[hfpef-phenotype-profiling]], [[hfpef-treatment-gap]]
- Entities: [[summit]], [[step-hfpef]], [[sglt2-inhibitors]]
- Sources: [[packer2025summit]], [[kramer2025summit-cmr]], [[kosiborod2023stephfpef]]

## Contradictions

No contradictions with overall trial. CKD consistency expected as weight loss mechanism is largely independent of renal function.
