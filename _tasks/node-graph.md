...
5. If more than 20 cross-references exist, limit displayed cross-refs to the 20 most-linked (by appearance count in the source page); note the cutoff in the build comment
...
2. **Extract and analyse**:
   ...
   - Propose cluster groupings (up to 6 clusters, each with a short uppercase label). If the user has already specified clusters, use those instead.
   - Propose a short descriptor for each inter-ref edge (e.g. "companion trial", "secondary analysis", "evidence base"). Keep descriptors ≤4 words.
...
6. **Write SVG** with these layers (bottom to top):
   ...
   - If edge descriptors are enabled: render each descriptor as a small label at the midpoint of its edge, `fill="#cbd5e1"` font-size 9, `text-anchor="middle"`, behind node circles
